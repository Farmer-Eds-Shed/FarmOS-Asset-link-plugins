...
export default {
  async onLoad(handle, assetLink) {
    ...
    handle.defineSlot('com.example.farmos_asset_link.actions.v0.increment_bale_inventory', action => {
      action.type('asset-action');

      action.showIf(({ asset }) => asset.attributes.status !== 'archived'
          // TODO: Implement a better predicate here...
          && asset.attributes.name.toLowerCase().indexOf("hay bales") !== -1);
      ...
      const doActionWorkflow = async (asset) => {
        ...
        const harvestLog = {
          type: 'log--harvest',
          attributes: {
            name: `Harvested ${baleCount} bale(s)`,
        ...
        assetLink.entitySource.update(
            ...
            {label: `Record new bales`});
      };

      action.component(({ asset }) =>
        h(QBtn, { block: true, color: 'secondary', onClick: () => doActionWorkflow(asset), 'no-caps': true },  "Record New Bales" ));
    });
  }
}
</script>
